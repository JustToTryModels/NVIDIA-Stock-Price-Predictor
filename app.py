...
def build_candlestick_chart(stock_data, predictions, prediction_dates, lookback_days=90):
    ...
    # Build custom data for conditional MA20/MA50 in tooltip
    customdata = []
    for i in range(len(df)):
        ma20_val = ma20.iloc[i]
        ma50_val = ma50.iloc[i]
        parts = []
        if not np.isnan(ma20_val) and i >= 19:
            parts.append(f"MA20 : ${ma20_val:.2f}")
        if not np.isnan(ma50_val) and i >= 49:
            parts.append(f"MA50 : ${ma50_val:.2f}")
        customdata.append("<br>".join(parts) if parts else "")

    # Custom hover template for candlestick - Date centered bold, OHLC, conditional MAs
    hovertemplate_candle = (
        "<b style='font-size:13px; text-align:center; display:block;'>%{x|%b %-d, %Y}</b><br><br>"
        "Open&nbsp;&nbsp;: $%{open:.2f}<br>"
        "High&nbsp;&nbsp;: $%{high:.2f}<br>"
        "Low&nbsp;&nbsp;&nbsp;: $%{low:.2f}<br>"
        "Close&nbsp;: $%{close:.2f}<br>"
    ...
    if predictions is not None and prediction_dates is not None:
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate='<b>Forecast</b><br>%{x|%b %-d, %Y}<br>Price: $%{y:.2f}<extra></extra>'
        ), row=1, col=1)
